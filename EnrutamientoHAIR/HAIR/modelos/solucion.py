from HAIR.modelos.graph import Graph
from constantes import constantes
from typing import Type
from .ruta import Ruta
from .penalty_variables import alpha, beta
from .tabulists import tabulists
from .mip1 import Mip1
from .mip2 import Mip2
from HAIR.tsp_local.base import TSP
from HAIR.tsp_local.kopt import KOpt
    ...
    if solucion != solucion_prima:
        ...
        for tiempo in range(constantes.horizon_length):
            ...
            for indice, cliente in enumerate(solucion_prima.rutas[tiempo].clientes):
                matriz[0][indice+1] = constantes.compute_distancia_proveedor(cliente.coord_x, cliente.coord_y)
                matriz[indice+1][0] = constantes.compute_distancia_proveedor(cliente.coord_x, cliente.coord_y)
...
class Solucion():
    ...
    def insertar_visita(self, cliente, tiempo):
        if constantes.politica_reabastecimiento == "OU":
            cantidad_delivered = cliente.nivel_maximo - self.obtener_niveles_inventario_cliente(cliente)[tiempo]
    ...
    def to_json(self):
        ...
        #     rutas_json += str(i)+":" + str(ruta.to_json())+','
        # print(rutas_json)
        return {"proveedor_id":str(constantes.proveedor.id), "rutas":{i:ruta.to_json() for i, ruta in enumerate(self.rutas)}}
