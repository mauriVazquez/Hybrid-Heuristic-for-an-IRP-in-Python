...
class Tour():
    ...
    def _makeEdges(self):
        ...
        >>> TSP.setEdges([[1] * 6] * 6)  # Dummy matriz
        >>> t = Tour([1, 2, 3, 4, 5])
    ...
    def indice(self, i):
        """
        Return the indice of a node in a tour.
        """
    ...
    def pred(self, indice, prev):
        """
        Return the predecessor or successor depending on the `prev` parameter.
        """
        return self.tour[indice - 1] if prev else self.tour[indice + 1]
    ...
    def generate(self, broken, joined):
        ...
        succ = successors[0]
        new_tour = [0]
        visitado = set(new_tour)

        # If we already encountered a node it means we have a loop
        while succ not in visitado:
            visitado.add(succ)
            new_tour.append(succ)
            succ = successors[succ]

        # If we visitado all nodes without a loop we have a tour
...
class KOpt(TSP):
    ...
    def _optimise(self):
        """
        Global loop which restarts at each improving solucion.
        """
        better = True
        self.solucions = set()
        ...
        # Restart the loop each tiempo we find an improving candidate
        while better:
            better = self.improve()
            # Paths always begin at 0 so this should manage to find duplicate
            # solucions
            self.solucions.add(str(self.heuristic_path))

        self.save(self.heuristic_path, self.heuristic_costo)
    ...
    def chooseX(self, tour, t1, last, gain, broken, joined):
        ...
        for t2i in around:
            ...
            if xi not in joined and xi not in broken:
                ...
                eliminado = deepcopy(broken)

                eliminado.add(xi)
                added.add(makePair(t2i, t1))  # Try to relink the tour

                relink = Gi - TSP.dist(t2i, t1)
                es_tour, new_tour = tour.generate(eliminado, added)

                # The current solucion does not form a valid tour
                if not es_tour and len(added) > 2:
                    continue

                # Stop the search if we come back to the same solucion
                if str(new_tour) in self.solucions:
                    Log.debug("Skip already found tour.")
                    return False

                Log.debug("Choose x: {} = {}".format(xi, TSP.dist(last, t2i)))

                # Save the current solucion if the tour is better, we need
                # `es_tour` again in the case where we have a non-sequential
                # exchange with i = 2
                if es_tour and relink > 0:
                    self.heuristic_path = new_tour
                    self.heuristic_costo -= relink

                    Log.info("Found {}-opt with gain {}".format(
                        len(added), relink))
                    Log.debug("X: {} - Y: {}".format(eliminado, added))

                    return True
                else:
                    # Pass on the newly "eliminado" edge but not the relink
                    choice = self.chooseY(tour, t1, t2i, Gi, eliminado, joined)
    ...
    def chooseY(self, tour, t1, t2i, gain, broken, joined):
        ...
        Parameters:
            ...
            - t2i: tail of the last edge eliminado (t_2i)
...
def test():
    ...
    t = KOpt(range(len(matriz)))
    t._optimise()

    return t.heuristic_path, t.heuristic_costo
